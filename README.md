...
<!-- Replace with an actual screenshot or the demo GIF once recorded -->
...
```
  Image + City
       ...
       ▼
┌─────────────────┐     ┌──────────────────────┐     ┌───────────────────┐     ┌────────────────┐
│  1. Detection    │────▶│  2. Verification      │────▶│  3. Response        │────▶│  4. Report      │
│  YOLOv8 (fire/   │     │  Cross-checks weather  │     │  RAG (LangChain +  │     │  PDF generation │
│  smoke) +        │     │  + live news for       │     │  FAISS + Gemini)   │     │  with full      │
│  ResNet18        │     │  plausibility           │     │  over NDMA disaster│     │  incident       │
│  (landslide)     │     │                        │     │  response docs      │     │  summary        │
└─────────────────┘     └──────────────────────┘     └───────────────────┘     └────────────────┘
```
...
A built-in **mutual suppression gate** handles the case where fire and landslide are both flagged on the same image (a known failure mode — see Limitations) by deciding final status through verification plausibility, not raw model confidence.
...
<!-- Embed the demo video/GIF here once recorded -->
